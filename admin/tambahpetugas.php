...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Cek duplikat data
    $stmt_cek = $pdo->prepare("SELECT id FROM petugas WHERE username = :username OR nip = :nip");
    $stmt_cek->execute([':username' => $username, ':nip' => $nip]);

    if ($stmt_cek->rowCount() > 0) {
        $pesan_error = "Username atau NIP sudah terdaftar!";
    } else {
        $stmt = $pdo->prepare("INSERT INTO petugas (nama, nip, username, password, jabatan, dinas_id)
                    ...
                    <form action="" method="POST" class="p-6 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="space-y-1.5 md:col-span-2">
                                ...
                                <input type="text" id="jabatan" name="jabatan" required placeholder="Cth: Staff IT"
                                    class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-colors text-sm text-dark placeholder:text-slate-400">
                            </div>
