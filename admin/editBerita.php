...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($pesan_error)) {
        $stmt_update = $pdo->prepare("UPDATE berita SET judul = :judul, kategori = :kategori, isi = :isi, foto = :foto WHERE id = :id");
        ...
        if ($success) {
            // Refresh data berita
            $stmt_refresh = $pdo->prepare("SELECT * FROM berita WHERE id = :id");
            $stmt_refresh->execute([':id' => $id_berita]);
            $berita = $stmt_refresh->fetch();
